...
def rungekutta4(x,y,z,dydx,dzdx,h,xstart,xend,flag=True):
    if (flag==True):  file = open("RK4"+ str(h)+".txt", "w+")
    T=[[],[]]
    a=x
    b=y
    c=z
...
def boundary(a,b,ya,yb,guess,dydx,dzdx,h,tol):
    ...
    if (X[1][-1]==yb):return X
        ...
        while(X[1][-1]>=yb):#Bracketing y(b)
            guess-= i
            X=rungekutta4(a,ya,guess,dydx,dzdx,h,a,b,False)
        #Converging to y(b) using lagrange extrapolation
        Cl=guess
        A=rungekutta4(a,ya,Ch,dydx,dzdx,h,a,b,False)
        B=rungekutta4(a,ya,Cl,dydx,dzdx,h,a,b,False)
        ych=A[1][-1]
        ycl=B[1][-1]
        C=Cl+(Ch-Cl)*(yb-ycl)/(ych-ycl)
        H=rungekutta4(a,ya,C,dydx,dzdx,h,a,b,False)
        yc=H[1][-1]
        while (abs(yc-yb)>=tol):
            if (yc>yb) :
                Ch=C
                ...
                yc=H[1][-1]
    n=len(H[0])
    #storing the contents of the list in a datafile
    file = open("Boundary"+ str(h)+".txt", "w+")
    for i in range(n):
        file.writelines([str(H[0][i])+"   ",str(H[1][i])+"\n"])
    file.close()
    return H
